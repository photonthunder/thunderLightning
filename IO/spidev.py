...
class SPIDev(object):
	def __init__(self, device, spimode=0):
		self._file = open(device, 'r+b', buffering=0)
		...
		ioctl(self._no, param, bytes([spimode]))
...
if __name__ == '__main__':
	myspi = SPIDev("/dev/spidev32765.0")
	transfer, bufOut, bufIn = spi_transfer(chr(0x1b), readlen = 0)
	myspi.do_transfers([tranfer])
	print(bufOut)
	print(bufIn)
